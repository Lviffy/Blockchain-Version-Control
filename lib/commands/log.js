        ...
        commits.forEach((commit, index) => {
          ...
          if (commit.files && commit.files.length > 0) {
            ...
            if (stats) {
              ...
              console.log(CLIUtils.colors.muted(`   Files: ${stats.totalFiles} file${stats.totalFiles !== 1 ? 's' : ''}${changeStr}`));
            } else {
              console.log(CLIUtils.colors.muted(`   Files: ${commit.files.length} file${commit.files.length !== 1 ? 's' : ''}`));
            }
            
            // Show first few files with their individual stats
            const filesToShow = commit.files.slice(0, 3);
            filesToShow.forEach((file, index) => {
              const size = file.size ? ` (${Math.round(file.size / 1024)} KB)` : '';
              
              // Show individual file stats if available
              if (stats && stats.fileStats && stats.fileStats[index]) {
                const fileStats = stats.fileStats[index];
                const changes = [];
                if (fileStats.additions > 0) changes.push(`+${fileStats.additions}`);
                if (fileStats.deletions > 0) changes.push(`-${fileStats.deletions}`);
                if (fileStats.modifications > 0) changes.push(`~${fileStats.modifications}`);
                
                const changeStr = changes.length > 0 ? ` (${changes.join(', ')})` : '';
                console.log(CLIUtils.colors.dim(`     • ${file.path}${size}${changeStr}`));
              } else {
