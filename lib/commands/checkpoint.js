...
checkpoint
  .description('Create a checkpoint to batch multiple commits')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (!hasBlockchain) {
        ...
        CLIUtils.info('Checkpoints are primarily for blockchain repositories.');
        CLIUtils.info('To enable blockchain features:');
        CLIUtils.info('  1. Run: bvc config --setup');
        CLIUtils.info('  2. Re-initialize with: bvc init --blockchain');
        return;
      }
      ...
      console.log(CLIUtils.colors.info(`Creating checkpoint for ${commitsInRange.length} commit(s)...`));
