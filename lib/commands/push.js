...
push
  .description('Push local commits to blockchain (individual transactions)')
  ...
  .addHelpText('after', `
Examples:
  $ bvc push                         Push all unpushed commits
  $ bvc push abc123                  Push specific commit
  $ bvc push --dry-run               Preview without gas fees
  $ bvc push --gas-price 20          Set custom gas price

⚠️  Cost Warning:
  • Push creates individual blockchain transactions for each commit
  • Use "bvc checkpoint" for cost-efficient batch operations
  • Consider: local commits → checkpoint → blockchain (recommended)
`)
  .action(async (commitHash, options) => {
    ...
    try {
      ...
      if (options.dryRun) {
        ...
        const estimatedGas = commitsToPush.length * 80000; // Rough estimate per commit
        console.log(CLIUtils.colors.muted(`\n📊 Estimated total gas: ~${estimatedGas} units`));
        console.log(CLIUtils.colors.warning(`💰 Individual transactions: ${commitsToPush.length} transactions × gas fees`));
        console.log(CLIUtils.colors.info(`💡 Alternative: Use "bvc checkpoint" for ${Math.round((1 - (estimatedGas/10)/(estimatedGas)) * 100)}% gas savings`));
        return;
      }

      // Push commits to blockchain
      spinner.text = `Pushing ${commitsToPush.length} commit(s) to blockchain...`;
      CLIUtils.warning(`⚠️  This will create ${commitsToPush.length} separate blockchain transactions`);
      CLIUtils.info(`💡 Consider "bvc checkpoint" for batch operations to reduce gas costs`);
...
${CLIUtils.icons.hash} Latest: ${commitsToPush[commitsToPush.length - 1].commitHash.substring(0, 8)}`, {
        ...
        CLIUtils.complete('Push completed!', [
          ...
          '💡 Future: Use "bvc checkpoint" for cost-efficient batch operations',
